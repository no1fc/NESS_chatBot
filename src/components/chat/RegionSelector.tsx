/**
 * RegionSelector 컴포넌트
 * 지역(시/도 → 시/군/구) 선택 UI 컴포넌트입니다.
 * 2단계 드롭다운으로 구성되며, 확인 버튼 클릭 시 지점 조회를 수행합니다.
 */
...
interface RegionSelectorProps {
    onSelect: (sido: string, sigungu: string) => void; // 지역 선택 완료 콜백
...
/**
 * RegionSelector 컴포넌트
 * 시/도 선택 후 해당 시/군/구 목록을 동적으로 표시합니다.
 */
export default function RegionSelector({ onSelect, disabled }: RegionSelectorProps) {
    ...
    const handleSidoChange = (sido: string) => {
        ...
        setSelectedSigungu(''); // 시/군/구 리셋
    };

    /**
     * 확인 버튼 클릭 핸들러
     * 두 값 모두 선택된 경우에만 콜백 호출
     */
    const handleConfirm = () => {
    ...
    return (
        <div
            className="flex flex-col gap-3 p-4 rounded-2xl animate-fade-in-up"
            style={{
                background: 'white',
        ...
        >
            ...
            <select
                ...
                style={{
                    border: '2px solid var(--color-gray-300)',
                    fontFamily: 'var(--font-sans)',
                    color: selectedSido ? 'var(--color-gray-900)' : 'var(--color-gray-400)',
                    background: 'white',
                    outline: 'none',
                    cursor: selectedSido ? 'pointer' : 'not-allowed',
                    opacity: selectedSido ? 1 : 0.6,
