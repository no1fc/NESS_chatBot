...
export async function POST(request: Request) {
    try {
        ...
        const { loginSchema, formatZodError } = await import('@/lib/validations');
        ...
        if (!adminUser) {
            ...
            if (username === FALLBACK_ADMIN_ID && password === FALLBACK_ADMIN_PASSWORD) {
                logger.info('초기 최고 관리자 계정 생성 (DB 삽입)');
                const salt = await bcrypt.genSalt(10);
                const hashedPassword = await bcrypt.hash(password, salt);

                // DB에 관리자 생성
                const created = await createAdmin(username, hashedPassword, 'superadmin');
