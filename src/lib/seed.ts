 ...
 * 최초 1회 실행하여 MSSQL DB에 지점 데이터를 삽입합니다.
 * 실행: npx tsx src/lib/seed.ts
 */

import { insertBranches, getBranchCount } from './db';
...
const branchData = [
    // 서울특별시
    {
        ...
        phone: '02-1234-5678',
        specific_url: 'https://docs.google.com/forms/d/1wFHHdMsFaRA_TO6rQ5hXhm8UUrM_3jZMYA-5ZvHLSw8/viewform?edit_requested=true',
    },
    {
        region_sido: '서울특별시',
        region_sigungu: '송파구',
...
async function runSeed(): Promise<void> {
    ...
    try {
        // 기존 데이터 개수 확인
        const existingCount = await getBranchCount();
        ...
        // 지점 데이터 일괄 삽입
        await insertBranches(branchData);

        // 삽입 결과 확인
        const newCount = await getBranchCount();
